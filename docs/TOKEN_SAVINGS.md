# Token Savings: ASTral MCP

## Why This Exists

AI agents waste tokens when they must read entire files to locate a single function, class, or constant.
ASTral indexes a repository once and allows agents to retrieve **exact symbols on demand**, eliminating unnecessary context loading.
...
| ASTral MCP       | ~1,449 tokens   | `search_symbols` > `get_symbol`       |
...
| Task                     | Raw Approach    | With ASTral     | Savings |
| ------------------------ | --------------- | --------------- | ------- |
...
ASTral shifts the workflow from:
...
No extra API calls or file reads — computed using fast file size checks only.
