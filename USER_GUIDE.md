...
To publish as a standalone tool:

```bash
dotnet publish src/ASTral/ASTral.csproj -c Release -o ./publish
```
...
* If not set, summaries fall back to docstrings or signatures.
...
The repository may not contain supported language files, or files may be excluded by skip patterns.
